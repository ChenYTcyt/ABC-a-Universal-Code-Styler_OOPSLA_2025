...
from .build.DFG import DFG_cpp,DFG_python,DFG_java,DFG_ruby,DFG_go,DFG_php,DFG_javascript,DFG_csharp
from .build.DFG import (remove_comments_and_docstrings,
                   tree_to_token_index,
                   index_to_code_token,
...
def compute_dataflow_match(tokenized_refs, tokenized_preds, refs, preds):
    lang = 'cpp'
    # calculate dataflow match
    dataflow_match_score = 0
    for ref, pred in zip(refs, preds):
        score = corpus_dataflow_match([[ref]], [pred], lang)
        # print(score)
        dataflow_match_score += score

    result = {
        'Dataflow Match': round(dataflow_match_score, 3)
    }
    return result